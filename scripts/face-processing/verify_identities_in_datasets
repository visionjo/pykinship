...
def to_html(imfiles_keep, imfiles_go):
    ...
    imagetable(cols, dout + 'face-cleanup.html', 'Sample Decision and Score',
...
for path_fid in tqdm(path_fids.glob('F????')):
    for path_mid in path_fid.glob('MID*'):
        ...
        imfile.sort()
        if len(imfile):
            imfile2 = [p for p in path_mid.joinpath('faces').glob('*.jpg') if not str(p).split('/')[-1][0].startswith('n')]
            imfile2.sort()
            fin = path_mid.joinpath(fn_features)
            with open(fin, 'rb') as f:
                encodings = pickle.load(f)
            keys_mid = [str(f).replace(path_fids._str, '').replace('faces/', '').replace('.jpg', '').replace('/F', 'F') for f in imfile2]
            keys_query = [str(f).replace(path_fids._str, '').replace('faces/', '').replace('.jpg', '').replace('/F', 'F') for f in imfile]

            encodings_mid = [encodings[k] for k in keys_mid]
            encodings_query_tup = [(k, encodings[k]) for k in keys_query if k in encodings]
            ...
            for f_tup, keep in zip(score_tup, mask):
                impath = path_fids.joinpath("/".join(f_tup[0].split('/')[:2]) + '/faces/' + f_tup[0].split('/')[-1] + '.jpg')
                if keep:
            ...
            for f_im in imkeep:
                ...
                if not path_out.is_file():
                    Path(f_im[0]).parent.parent.joinpath('faces-cleaned').mkdir(exist_ok=True)
