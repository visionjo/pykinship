...
def to_html(imfiles_keep, imfiles_go, mid):
    ...
    cols = [
        ...
        Col('img', 'keep', [f.replace(dout, '') for f, _ in imfiles_keep], None, 'background: #28aade', href=[f.replace(dout, '') for f, _ in imfiles_keep]),
        Col('text', 'keep (score)', scores_keep),
        Col('img', 'dump', [f.replace(dout, '') for f, _ in imfiles_go], None, 'background: #db8a0f', href=[f.replace(dout, '') for f, _ in imfiles_go]),
        Col('text', 'dump (score)', scores_go, style='text {width: 2in;}'),
    ]
    summary_row = ['ID', len(imfiles_keep), np.mean([float(sc) for _, sc in imfiles_keep]), len(imfiles_go), np.mean([float(sc) for _, sc in imfiles_go])]

    imagetable(cols, dout + f'{mid}-face-cleanup.html', 'Sample Decision and Score',
...
for path_fid in tqdm(path_fids.glob('F????')):
    ...
    for path_mid in path_fid.glob('MID*'):
        ...
        imfile.sort()
        if len(imfile):
            imfile2 = [p for p in path_mid.glob('*.jpg') if not str(p).split('/')[-1][0].startswith('n')]
            imfile2.sort()
            fin = path_mid.joinpath('encodings').joinpath(fn_features)
            with open(fin, 'rb') as f:
                encodings = pickle.load(f)
            keys_mid = [str(f).replace(path_fids._str, '').replace('/F', 'F') for f in imfile2]
            keys_query = [str(f).replace(path_fids._str, '').replace('/F', 'F') for f in imfile]
            ...
            for k in keys_query:
                if k in encodings:
                    if len(encodings[k].numpy()) > 1:
                        ...
                        encodings_query_tup.append((k, (encodings[k].numpy().squeeze())))
            ...
            scores_matrix = cosine_similarity(np.array([e[1] for e in encodings_query_tup]), np.array([ee for ee in encodings_mid]))
            # scores_matrix = cosine_similarity([e[1] for e in encodings_query_tup], encodings_mid)
            scores = np.median(scores_matrix, axis=1)
            mask = scores > .2

            score_tup = np.array([(f, sc) for f, sc in zip([e[0] for e in encodings_query_tup], scores)])
            imkeep, imdrop = [], []
            impath = None
            for f_tup, keep in zip(score_tup, mask):
                impath = path_fids.joinpath("/".join(f_tup[0].split('/')[:2]) + '/' + f_tup[0].split('/')[-1])
            ...
            for f_im in imkeep:
                ...
                shutil.copyfile(f_im[0], Path(f_im[0]).parent.joinpath('faces-cleaned').joinpath(f_im[0].split('/')[-1]))
