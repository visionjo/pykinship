...
for f_video in tqdm(f_videos):
    ...
    try:
        ...
        print("skipping", f_video, e.message)
        continue
    ...
    for k, frame in enumerate(clip.iter_frames(fps=fps)):
        ...
        if (
            ...
            for i in range(ndetections):
                ...
                path_image_out = path_obin / str(Path(path_image_out).name).replace(
            ...
            print(
                "{} is discarded due to non-detected landmarks!".format(path_image_out)
            )
