...
video_paths = [v for v in dir_wav.glob("F????/MID*/*/*/*.mp4") if "clips" not in str(v)]
video_paths.sort()
# vfiles.sort()
...
for tfile in reversed(tfiles):
    ...
    obin = dir_out
    obin = obin / (
        "/".join(str(tfile).replace(str(dir_wav) + "/", "").split("/")[:2])
        + "/clips/"
        + youtube_id
        + "/"
    )
    ...
    for i in range(nclips):
        ...
        ofile = str(
            obin / (str(df.iloc[i]["ref"].split("/")[-1]).split("_")[-1] + ".mp4")
        )
        ...
        os.system(
            f"ffmpeg -i {vfile} -ss {start} -t {span} -r {frame_rate} -async 1 -qscale:v 5 -deinterlace -vf scale=-1:360  {ofile}"
        )
