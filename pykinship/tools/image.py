...
from pykinship.tools.io import mkdir
from pykinship.tools import io
from pykinship.tools.common import is_numpy
...
def list(imdir):
    ...
    return [
        os.path.abspath(os.path.join(imdir, item))
        for item in os.listdir(imdir)
        if (io.is_img(item) and not io.is_hidden_file(item))
    ]
...
def writegray(img, imfile=None):
    ...
    if is_numpy(img):
        if img.dtype == np.dtype("uint8"):
            ...
            raise ValueError(
                "Unsupported datatype - Numpy array must be uint8 or float32"
            )
    else:
        raise ValueError("Input must be numpy array")
    return imfile
...
def write(img, imfile=None, writeas=None):
    ...
    if writeas in ["jet"]:
        ...
        if img.dtype == np.dtype("uint8"):
            ...
            cv2.imwrite(
                imfile, rgb2bgr(np.uint8(255.0 * img))
