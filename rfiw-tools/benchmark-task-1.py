...
print('Processing {} pairs of {} relationship types'.
      format(len(df_pairlist), len(relationship_types)))
...
f, axs = plt.subplots(2, int(np.ceil(len(relationship_types) / 2)),
                      figsize=(15, 7), sharex=True, sharey=True)
i = [[0, 0], [0, 1], [0, 2], [0, 3], [0, 4], [1, 0], [0, 5], [1, 1], [1, 2],
     [1, 3], [1, 4], [1, 5]]
for j, att in enumerate(relationship_types):
    df_cur = df_pairlist.loc[df_pairlist.ptype == att, ['score', 'label']]
    sns.distplot(df_cur.loc[df_cur.label == 1, 'score'], hist=True,
                 label='True', ax=axs[i[j][0], i[j][1]], color='g')
    sns.distplot(df_cur.loc[df_cur.label == 0, 'score'], hist=True,
                 label='False', ax=axs[i[j][0], i[j][1]], color='r')
...
sns.violinplot(x='ptype', y='score', data=df_pairlist, hue='tags', ax=ax,
               linewidth=2.5, width=0.75, palette="Pastel1",
               order=["BB", "SS", "SIBS", 'FD', 'FS', "MD", "MS", 'GFGD',
                      'GFGS', 'GMGD', 'GMGS'])
...
ax = fig.gca()
sns.despine(left=True)
df_pairs = df_pairlist.loc[df_pairlist.ptype.str.contains('G') == False]
sns.violinplot(x='ptype', y='score',
               data=df_pairs, hue_order=['NON-KIN', 'KIN'], split=True,
               hue='tags', ax=ax, linewidth=2.5, width=0.75, palette="Pastel1",
